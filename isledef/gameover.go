...
func CreateGameOver(g *Game, win bool) *GameOver {
	...
	t := &GameOver{
		title: tentsuyu.NewTextElementStationary(288, 5, 600, 200, tentsuyu.Components.ReturnFont(FntSmallPixel), []string{"Game Over!", "You ran out of bombs!"}, color.White, 16),
	}
	t.win = win
	if TimeRanOut {
		t.title = tentsuyu.NewTextElementStationary(288, 5, 600, 200, tentsuyu.Components.ReturnFont(FntSmallPixel), []string{"Game Over!", "You ran out of time!"}, color.White, 16)
	}
	if g.gameData.gameMode == GameModeNormalBattle {
		t.title = tentsuyu.NewTextElementStationary(288, 5, 600, 200, tentsuyu.Components.ReturnFont(FntSmallPixel), []string{"Game Over!", "Nure Sank Your Monsters!!"}, color.White, 16)
	}
	if win {
	...
	testMenu.AddElement([]tentsuyu.UIElement{tentsuyu.NewTextElement(0, 0, 200, 30, tentsuyu.Components.ReturnFont(FntSmallPixel), []string{"Play Again"}, color.White, 16)},
		[]func(){func() {
			if g.gameData.gameMode == GameModeHardcoreTimed || g.gameData.gameMode == GameModeNormalTimed {
